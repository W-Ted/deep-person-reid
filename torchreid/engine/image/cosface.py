...
from torchreid.losses import Am_softmax
from torchreid.utils import AverageMeter, open_specified_layers, open_all_layers
from torchreid import metrics


class ImageCosfaceEngine(engine.Engine):
	def __init__(self, datamanager, model, optimizer, scheduler=None, use_cpu=False,
	             label_smooth=True):
		super(ImageCosfaceEngine, self).__init__(datamanager, model, optimizer, scheduler, use_cpu)

		self.criterion = Am_softmax(
			classnum=self.datamanager.num_train_pids,
		...
		for batch_idx, data in enumerate(trainloader):
			...
			if (batch_idx + 1) % print_freq == 0:
				print('Epoch: [{0}][{1}/{2}]\t'
				      ...
				      'Cosface Loss {loss.val:.4f} ({loss.avg:.4f})\t'
