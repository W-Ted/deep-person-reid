...
from torchreid.losses import CrossEntropyLoss, HistogramLoss
from torchreid.utils import AverageMeter, open_specified_layers, open_all_layers
from torchreid import metrics


class ImageHistogramEngine(engine.Engine):

	def __init__(self, datamanager, model, optimizer, scheduler=None, use_cpu=False,
	             label_smooth=True):
		super(ImageHistogramEngine, self).__init__(datamanager, model, optimizer, scheduler, use_cpu)

		self.criterion = HistogramLoss(
		...
		for batch_idx, data in enumerate(trainloader):
			...
			if (batch_idx + 1) % print_freq == 0:
				print('Epoch: [{0}][{1}/{2}]\t'
				      ...
				      'Histogram Loss {loss.val:.4f} ({loss.avg:.4f})\t'
