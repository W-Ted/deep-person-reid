...
from torchreid.losses import Arcface
...
class ImageArcfaceEngine(engine.Engine):
	def __init__(self, datamanager, model, optimizer, scheduler=None, use_cpu=False,
	             label_smooth=True):
		super(ImageArcfaceEngine, self).__init__(datamanager, model, optimizer, scheduler, use_cpu)
		...
		for batch_idx, data in enumerate(trainloader):
			...
			if (batch_idx + 1) % print_freq == 0:
				print('Epoch: [{0}][{1}/{2}]\t'
				      ...
				      'Arcface Loss {loss.val:.4f} ({loss.avg:.4f})\t'
