...
from torchreid.losses import AngleLoss, AngleLoss_nomargin
from torchreid.utils import AverageMeter, open_specified_layers, open_all_layers
from torchreid import metrics

class ImageSphereEngine(engine.Engine):
	def __init__(self, datamanager, model, optimizer, scheduler=None, use_cpu=False,
				 label_smooth=True):
		super(ImageSphereEngine, self).__init__(datamanager, model, optimizer, scheduler, use_cpu)
		...
		#self.criterion = AngleLoss_nomargin(
		#	num_classes=self.datamanager.num_train_pids,
		#	use_gpu=self.use_gpu,
		#	label_smooth=label_smooth
		#)
		...
		for batch_idx, data in enumerate(trainloader):
			...
			if (batch_idx + 1) % print_freq == 0:
				print('Epoch: [{0}][{1}/{2}]\t'
					  ...
					  'A-softmax Loss {loss.val:.4f} ({loss.avg:.4f})\t'
